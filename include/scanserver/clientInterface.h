...
protected:
  ...
  ip::offset_ptr<ScanVector> m_scanvector_ptr;
...
public:
  ...
  ScanVector* readDirectory(const char* dir_path, IOType type = UOS, unsigned int start = 0, unsigned int end = -1);

  //! "Close" a directory by releasing all contained scans, deleting the vector and resetting the pointer
  void closeDirectory(ScanVector*& scans);

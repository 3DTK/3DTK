...
public:
  //range_set(std::string ranges_def);
  //void set(std::string merge_def);

  range_set(const std::vector<std::vector<range<T> > >& subsets = {}
    , const range<T>& limits = {0, -1, 1, 1})
    //, const T& cluster_size = 1)
    //, const unsigned int& stepsize = 1)
    : m_subsets(subsets) //, m_cluster_size(cluster_size) //, m_stepsize(stepsize)
  {
    setLimits(limits);
  ...
  inline void setLimits(const range<T>& limits) { 
    m_limits = limits;
    //if (limits.min == -1 && m_subsets.size() > 0) {
    //  auto last = m_subsets.back();
    //  m_limits.max = last.back().max;
    //}
  }
  inline void setSubsets(const std::vector<std::vector<range<T> > >& subsets) { m_subsets = subsets; }
  //inline void setClusterSize(T cluster_size) { m_cluster_size = cluster_size; }
  //inline void setStepsize(T stepsize) { m_limits.stepsize = stepsize; }
...
private:
  ...
  inline T next_range() {
    ...
    if (subset_index[1] < currentSubset().size() - 1) {
      ...
      //    subset_index[0]++;
      //    return pos = ceil_pos(currentSubset()[0].min);
      //  }
      //  else {
      //    //pos = floor_pos(currentRange().max);
      //    in_subset = false;
      //    subset_done = reached_end = true;
      //    return pos;
      //  }
      //}
      //pos = ceil_pos(currentSubset()[0].max + 1);
      //subset_done = true;
      //in_subset = false;
      //return pos;
    }

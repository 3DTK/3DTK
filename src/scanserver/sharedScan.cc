...
void SharedScan::setRangeParameters(float max_dist, float min_dist)
{
...
const FrameVector& SharedScan::getFrames(bool enforce_loading)
{
  // on a restart with existing frame files try to load these
  if((m_frames.empty() && m_load_frames_file == true) || enforce_loading) {
    ClientInterface* client = ClientInterface::getInstance();
...
CacheDataAccess SharedScan::getOcttree()
{
  return m_octtree->getCacheData<SharedScan::onCacheMiss>();
}

CacheDataAccess SharedScan::createOcttree(unsigned int size)
{

...
ScanVector * ClientInterface::readDirectory(const char * dir_path, IOType type, unsigned int start, unsigned int end)
{
  ...
  ScanVector* scans = m_scanvector_ptr.get();
  m_scanvector_ptr = 0;
  return scans;
}

void ClientInterface::closeDirectory(ScanVector*& scans)
{
  if(scans == 0) return;

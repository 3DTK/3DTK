...
#include "scanserver/scan_io/scan_io.h"
...
ScanVector* ServerInterface::readDirectory(const char * dir_path, IOType type, unsigned int start, unsigned int end)
{
  // INFO
  ...
  ScanVector* scans = segment_manager->construct<ScanVector>(anonymous_instance)(allocator);
...
SharedScan* ServerInterface::findScan(const SharedStringSharedPtr& dir_path, const char* identifier, IOType type) const
{
  ...
  for(ScanVector::const_iterator it = m_scans.begin(); it != m_scans.end(); ++it) {
    if(scan == *(*it))
      return it->get();
  }
  return 0;

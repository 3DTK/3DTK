...
floorplan::Plane3d::Plane3d() {
    // nothing to do
}

floorplan::Plane3d::Plane3d(const Point3d& pt, const Vector3d& normal) {
    this->pt     = pt;
    this->normal = normal;
...
floorplan::Plane3d::Plane3d(const Plane3d& other) {
    this->pt     = other.pt;
...
floorplan::Point3d floorplan::Plane3d::intersect(const Plane3d& second, const Plane3d& third) {
...
floorplan::Plane3d& floorplan::Plane3d::operator=(const Plane3d& other) {
    this->pt     = other.pt;
    this->normal = other.normal;
    this->hull   = other.hull;
...
bool floorplan::Plane3d::isVertical() {
    this->normal.normalize();
    return (fabs(this->normal.y) < _EPSILON_VERTICAL);
}

bool floorplan::Plane3d::isHorizontal() {
    this->normal.normalize();
    return ((1 - fabs(this->normal.y / this->normal.length())) < _EPSILON_HORIZONTAL);
}

bool floorplan::Plane3d::isSamePlane(floorplan::Plane3d other) {
    // just to be safe
    this->normal.normalize();
...
floorplan::Vector3d floorplan::Plane3d::computeAverageNormal() {
    if(this->hull.size() < 3) {
...
vector<vector<floorplan::Point3d> > floorplan::Plane3d::getDiscretePoints(const double& dist) const {
    if (this->hull.size() < 3) {
    ...
    vector<vector<floorplan::Point3d> > result;

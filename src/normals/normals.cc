                   ...
                   IOType &iotype, int &k1, int &k2, normal_method &ntype,int &width,int &height)
{
	...
	cout << " bin/calculateNormals -s 0 -e 0 -f UOS -g PANORAMA_FAST dat/" <<endl;
	cout << endl << endl;
...
///////////////////////////////////////////////////////
void calculateNormalsAKNN(vector<Point> &normals,vector<Point> &points, int k )
{
	...
	for (size_t i=0; i<points.size(); ++i)
		...
		t.annkSearch(p, nr_neighbors, n, d, 0.0);
		...
		for (int i = 0; i < nr_neighbors; ++i) {
		  X(i+1, 1) = points[n[i]].x - mean.x;
		  X(i+1, 2) = points[n[i]].y - mean.y;
		  X(i+1, 3) = points[n[i]].z - mean.z;
		}
		...
		temp_n[1] = U(2,1);
		temp_n[2] = U(3,1);
		norm_n = sqrt(temp_n[0]*temp_n[0] + temp_n[1]*temp_n[1] + temp_n[2]*temp_n[2]);
		temp_n[0] /= norm_n;
		temp_n[1] /= norm_n;
		temp_n[2] /= norm_n;
		
            	normals.push_back(Point(temp_n[0],temp_n[1],temp_n[2]));  	
        }
	delete[] n;
	delete[] d;
}
...
////////////////////////////////////////////////////////////////
void calculateNormalsAdaptiveAKNN(vector<Point> &normals,vector<Point> &points, int kmin, int kmax)
{
	cout<<"Total number of points: "<<points.size()<<endl;
	int nr_neighbors;	
	ANNpointArray pa = annAllocPts(points.size(), 3);
	...
	for (size_t i=0; i<points.size(); ++i)
	  ...
	  Matrix U(3,3);
		for(int kidx = kmin; kidx <= kmax;kidx++)
			...
			//is atleast 25 percent of the maximum eigen value 
			if ( (e2 > 0.25 * e1) && (fabs(1.0 - (double)e2/(double)e3) < 0.25)) 
				break;
		}	
		//normal = eigenvector corresponding to lowest 
...
///////////////////////////////////////////////////////
void calculateNormalsPANORAMA(vector<Point> &normals,vector<Point> &points,vector<vector<vector<cv::Vec3f>>> extendedMap)
{
	cout<<"Total number of points: "<<points.size()<<endl;
	points.clear();
	int nr_neighbors = 0;
	...
	for (size_t i=0; i< extendedMap.size(); i++)
	  ...
	  for (size_t j=0; j<extendedMap[i].size(); j++)
	    {
	      //	      cout << i << " " << j << " " << extendedMap[i][j].size() << endl;
		...
		X(i+1, 3) = p[2] - mean.z;
	      ...
	      for(int k = 0; k < extendedMap[i][j].size(); k++)
		{
...
void orientNormals2Pose(vector<Point> &normals, double tempR[16])
{
	Matrix RT(4,4);
	...
	for(int i=0;i<4;i++)
	{
		for(int j=0;j<4;j++)
		{
			RT(i+1,j+1) = tempR[i*4 + j%4];
			cout << RT(i+1,j+1) << " ";
		}
		cout << endl;
	}	
	int n = normals.size();
	Matrix in(4,1),fn(4,1); 	
	for(int i = 0;i<n;i++)
	{
		in(1,1) = normals[i].x;
		in(2,1) = normals[i].y;
...
int main(int argc, char** argv)
{
	...
	if(ntype == AKNN)
		calculateNormalsAKNN(normals,points, k1 );
	else if(ntype == ADAPTIVE_AKNN)	
		calculateNormalsAdaptiveAKNN(normals,points, k1,k2 );
	else 
	  {
	    // create panorama
	    ...
	    if(ntype == PANORAMA)
	      calculateNormalsPANORAMA(normals,points,fPanorama.getExtendedMap());
	    // else if(ntype == PANORAMA_FAST)
	    //   calculateNormalsFAST(normals,points,img,fPanorama.getExtendedMap());
	  }

	//to orient normal towards pose of the observation point
	double tempR[16];
    	EulerToMatrix4(rPos, rPosTheta, tempR);	
	//	orientNormals2Pose(normals,tempR);

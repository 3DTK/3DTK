		...
		)
{
	...
	double tDeltaX, stepX, tMaxX;
	double tDeltaY, stepY, tMaxY;
	double tDeltaZ, stepZ, tMaxZ;
	if (direction[0] == 0) {
		tDeltaX = 0;
		stepX = 0;
		...
		tMaxX = tDeltaX * (1.0 - get_fraction(stepX*start[0]/voxel_size));
	}
	if (direction[1] == 0) {
		tDeltaY = 0;
		stepY = 0;
		...
		tMaxY = tDeltaY * (1.0 - get_fraction(stepY*start[1]/voxel_size));
	}
	if (direction[2] == 0) {
		tDeltaZ = 0;
		stepZ = 0;
		...
		tMaxZ = tDeltaZ * (1.0 - get_fraction(stepZ*start[2]/voxel_size));
	}
	...
	while (X != endX || Y != endY || Z != endZ) {
		...
		if (diff == 0) {
			...
			for (std::set<size_t>::iterator it = scanslices->second.begin(); it != scanslices->second.end(); ++it) {
				if (*it >= current_slice - diff && *it <= current_slice + diff) {
					found = true;
					break;
				}
			}
...
int main()
{
	...
	Scan::openDirectory(false, "../wolfsburg_mim_registered", UOSR, 120000, 155000);

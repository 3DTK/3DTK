...
public:
  ...
  LASoperationChangeClassificationFromTo(U8 class_from, U8 class_to) { this->class_from = (class_from & 31); this->class_to = (class_to & 31); };
private:
  U8 class_from;
  U8 class_to;
};
...
BOOL LAStransform::parse(int argc, char* argv[])
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if (strncmp(argv[i],"-set_classification", 19) == 0)
    {
      ...
      add_operation(new LASoperationSetClassification((U8)atoi(argv[i+1])));
      *argv[i]='\0'; *argv[i+1]='\0'; i+=1; 
    }

...
ImVector<vgm::Vec3> imguiGizmo::sphereVtx;
ImVector<int>  imguiGizmo::sphereTess;
ImVector<vgm::Vec3> imguiGizmo::arrowVtx[4];
ImVector<vgm::Vec3> imguiGizmo::arrowNorm[4];
ImVector<vgm::Vec3> imguiGizmo::cubeVtx;
ImVector<vgm::Vec3> imguiGizmo::cubeNorm;
ImVector<vgm::Vec3> imguiGizmo::planeVtx;
ImVector<vgm::Vec3> imguiGizmo::planeNorm;
...
// Axes resize
///////////////////////////////////////
vgm::Vec3 imguiGizmo::axesResizeFactor(.95f, 1.0f, 1.0f);
vgm::Vec3 imguiGizmo::savedAxesResizeFactor = imguiGizmo::axesResizeFactor;
...
//  Quaternion control 
//      in/out:  
//          - vgm::Quat (quaternion) rotation
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Quat& q, float size, const int mode)
{
    imguiGizmo g;
...
//  Angle/Axes control 
//      in/out: 
//          - vgm::Vec4 - X Y Z vector/axes components - W angle of rotation
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Vec4& axis_angle, float size, const int mode)
{
    imguiGizmo g;
    g.modeSettings(mode & ~g.modeDual);
...
//  Direction control  
//      in/out: 
//          - vgm::Vec3 - X Y Z vector/axes components
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Vec3& dir, float size, const int mode)
{
    imguiGizmo g;
...
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Quat& axes, vgm::Quat& spot, float size, const int mode)
{
    imguiGizmo g;
    g.setDualMode(mode);
...
//  2 Manipulators -> Quaternion and vgm::Vec3
//      in/out: 
//          - axes (quaternion) for full control - LeftClick 
//          - spot (vgm::Vec3)       for full control - RightClick
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Quat& axes, vgm::Vec3& spotDir, float size, const int mode)
{
    imguiGizmo g;
    g.setDualMode(mode);
...
//  2 Manipulators -> Quaternion and vgm::Vec4
//      in/out: 
//          - axes (quaternion) for full control - LeftClick 
//          - spot (vgm::Vec4)       for full control - RightClick
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Quat& axes, vgm::Vec4& axesAngle, float size, const int mode)
{
    imguiGizmo g;
    g.setDualMode(mode);
...
//      in/out: 
//          - vgm::Vec3 Pan(x,y) Dolly(z)
//          - vgm::Quat (quaternion) rotation
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Vec3& vPanDolly, vgm::Quat& q, float size, const int mode)
{
    imguiGizmo g;
...
//          - vgm::Vec4 - X Y Z vector/axes components - W angle of rotation
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Vec3& vPanDolly, vgm::Vec4& axis_angle, float size, const int mode)
{
    imguiGizmo g;
...
//          - vgm::Vec3 - X Y Z vector/axes components
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Vec3& vPanDolly, vgm::Vec3& dir, float size, const int mode)
{
    imguiGizmo g;
...
//      in/out: 
//          - vgm::Vec3 Pan(x,y) Dolly(z) - default: Ctrl /Shift
//          - axes (quaternion) for full control - LeftClick 
//          - spot (quaternion) for full control - RightClick
//
//                both pressed buttons... rotate together
...
//                are abilitated on both ... also together!
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Vec3& vPanDolly, vgm::Quat& axes, vgm::Quat& spot, float size, const int mode)
{
    imguiGizmo g;
    g.setDualMode(mode);
    g.posPanDolly = vPanDolly;
...
//  2 Manipulators -> Quaternion and vgm::Vec3  + Pan & Dolly
//      in/out: 
//          - vgm::Vec3 Pan(x,y) Dolly(z) - default: Ctrl /Shift
//          - axes (quaternion) for full control - LeftClick 
//          - spot (vgm::Vec3)       for full control - RightClick
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Vec3& vPanDolly, vgm::Quat& axes, vgm::Vec3& spotDir, float size, const int mode)
{
    imguiGizmo g;
    g.setDualMode(mode);
    g.posPanDolly = vPanDolly;
...
//  2 Manipulators -> Quaternion and vgm::Vec4  + Pan & Dolly
//      in/out: 
//          - vgm::Vec3 Pan(x,y) Dolly(z) - default: Ctrl /Shift
//          - axes (quaternion) for full control - LeftClick 
//          - spot (vgm::Vec4)       for full control - RightClick
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, vgm::Vec3& vPanDolly, vgm::Quat& axes, vgm::Vec4& axesAngle, float size, const int mode)
{
    imguiGizmo g;
    g.setDualMode(mode);
    g.posPanDolly = vPanDolly;
...
////////////////////////////////////////////////////////////////////////////
inline ImU32 addLightEffect(const vgm::Vec4 &color, float light, float atten)
{                          
    vgm::Vec3 l((light<.5) ? .5f : light); 
    vgm::Vec3 a(atten>.25  ? .25f : atten);
    vgm::Vec3 c(((vgm::Vec3(color) + l*.5f) * l) *.75f + a*vgm::Vec3(color)*.45f +a*.25f);
...
{                        
    vgm::Vec4 c(float(color & 0xff)/255.f,float((color>>8) & 0xff)/255.f,float((color>>16) & 0xff)/255.f, 1.0f);
    return addLightEffect(c, light, atten);
}

//  inline helper drawing functions
////////////////////////////////////////////////////////////////////////////
typedef vgm::Vec3 & (*ptrFunc)(vgm::Vec3 &);


inline vgm::Vec3 &adjustPlane(vgm::Vec3 &coord)
{
    ...
    coord *= vgm::Vec3(1.0f, 2.0f, 2.0f);
    return coord;
}

inline vgm::Vec3 &adjustDir(vgm::Vec3 &coord)
{
    coord.x = (coord.x > 0.0f) ? ( 2.5f * coord.x - 1.6f) : coord.x + 0.1f;
    coord *= vgm::Vec3(1.0f, 3.0f, 3.0f);
    return coord;
}

inline vgm::Vec3 &adjustSpotCyl(vgm::Vec3 &coord)
{
...
inline vgm::Vec3 &adjustSpotCone(vgm::Vec3 &coord)
{
    coord.x-= 2.00f;
    return coord;
}

inline vgm::Vec3 fastRotate (int axis, vgm::Vec3 &v)
{
    return ((axis == imguiGizmo::axisIsY) ? vgm::Vec3(-v.y, v.x, v.z) : // rotation Z 90'
           ((axis == imguiGizmo::axisIsZ) ? vgm::Vec3(-v.z, v.y, v.x) : // rotation Y 90'
                                          v));
}
////////////////////////////////////////////////////////////////////////////
//
...
bool imguiGizmo::drawFunc(const char* label, float size)
{
    ...
    vgm::Vec3 resizeAxes( ((drawMode&modeDual) && (axesResizeFactor.x>.75f)) ? vgm::Vec3(.75f,axesResizeFactor.y, axesResizeFactor.z) : axesResizeFactor);
    ...
    auto addTriangle = [&] ()
    {   // test cull dir        
        if(cross(vgm::Vec2(uv[1].x-uv[0].x, uv[1].y-uv[0].y), 
                 vgm::Vec2(uv[2].x-uv[0].x, uv[2].y-uv[0].y)) > 0) { uv[1] = uv[2] = uv[0]; }
    ...
    {   // test cull dir
        if(cross(vgm::Vec2(uv[1].x-uv[0].x, uv[1].y-uv[0].y), 
                 vgm::Vec2(uv[3].x-uv[0].x, uv[3].y-uv[0].y)) > 0) { uv[3] = uv[1] = uv[2] = uv[0]; }
    ...
    auto drawSphere = [&] () 
    {
        ...
        for(auto itVtx = sphereVtx.begin(); itVtx != sphereVtx.end(); )  {
            for(int h=0; h<3; h++, itTess++) {
                vgm::Vec3 coord = _q  * (*itVtx++ * solidResizeFactor);        //Rotate
    ...
    auto drawCube = [&] ()  
    {
        ...
        for(vgm::Vec3* itNorm = cubeNorm.begin(), *itVtx  = cubeVtx.begin() ; itNorm != cubeNorm.end();) {
            vgm::Vec3 coord;
            vgm::Vec3 norm = _q * *itNorm;
            for(int i = 0; i<4; ) {
                coord = _q  * (*itVtx++ * solidResizeFactor);
                uv[i++] = normalizeToControlSize(coord.x,coord.y);
            }                    
            addQuad(addLightEffect(vgm::Vec4(abs(*itNorm++),1.0f), norm.z, coord.z));
        }
    };
    ...
    auto drawPlane = [&] ()  
    {
        ...
        for(auto itNorm = planeNorm.begin(), itVtx  = planeVtx.begin() ; itNorm != planeNorm.end();) {
            ...
            itNorm++;
            addQuad(addLightEffect(vgm::Vec4(planeColor.x, planeColor.y, planeColor.z, planeColor.w), norm.z, coord.z));
        }
    };
    ...
    {   
        for(int n = 0; n < 4; n++) { //Arrow: 2 Cone -> (Surface + cap) + 2 Cyl -> (Surface + cap)
            for(int arrowAxis = 0; arrowAxis < 3; arrowAxis++) { // draw 3 axes
                vgm::Vec3 arrowCoord(0.0f, 0.0f, 0.0f); arrowCoord[arrowAxis] = 1.0f; // rotate on 3 axis (arrow -> X, Y, Z ) in base to current arrowAxis
                const float arrowCoordZ = vgm::Vec3(_q*arrowCoord).z; //.Rotate
                ...
                for(auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm+i)->begin(); itVtx != ptrVtx->end(); ) { //for all Vtx
                    ...
                    vgm::Vec3 norm( _q * fastRotate(arrowAxis, *itNorm++));
#endif
                    for(int h=0; h<3; h++) {
                        vgm::Vec3 coord(*itVtx++ * resizeAxes); //  reduction
                    ...
                    //transform
                        ...
                        col[h] = addLightEffect(vgm::Vec4(float(arrowAxis==axisIsX),float(arrowAxis==axisIsY),float(arrowAxis==axisIsZ), 1.0), norm.z, coord.z);
                    }
                    addTriangle();
                }
            }
    ...
    //////////////////////////////////////////////////////////////////
    auto drawComponent = [&] (const int idx, const vgm::Quat &q, ptrFunc func)
    {
...
#endif
            for(int h=0; h<3; h++) {
                vgm::Vec3 coord = *itVtx++;
#ifdef imguiGizmo_INTERPOLATE_NORMALS
                vgm::Vec3 norm = (q * *itNorm++);
#endif
                ...
                col[h] = addLightEffect(vgm::Vec4(directionColor.x, directionColor.y, directionColor.z, 1.0), norm.z, coord.z>0 ? coord.z : coord.z*.5);
            }
            addTriangle();
        }
    };
    ...
    //////////////////////////////////////////////////////////////////
    auto dirArrow = [&] (const vgm::Quat &q, int mode) 
    {
        vgm::Vec3 arrowCoord(_q * vgm::Vec3(1.0f, 0.0f, 0.0f));
    ...
    //////////////////////////////////////////////////////////////////
    auto spotArrow = [&] (const vgm::Quat &q, const float arrowCoordZ) 
    {
        if(arrowCoordZ > 0) { 
...
#define CENTER_HELPER_Y -.85f
    ...
    if(drawMode & (modeDirection | modeDirPlane)) dirArrow(_q, drawMode);
        ...
        if(drawMode & modeDual) {
            vgm::Vec3 spot(qtV2 * vgm::Vec3(-1.0f, 0.0f, .0f)); // versus opposite
...
//  Polygon
////////////////////////////////////////////////////////////////////////////
void imguiGizmo::buildPolygon(const vgm::Vec3 &size, ImVector<vgm::Vec3> &vtx, ImVector<vgm::Vec3> &norm)
{

    vtx .clear();
    norm.clear(); 

#define V(a,b,c) vtx.push_back(vgm::Vec3(a size.x, b size.y, c size.z))
#define N(x,y,z) norm.push_back(vgm::Vec3(x, y, z))
...
#   define V(x,y,z) sphereVtx.push_back(vgm::Vec3(x, y, z))
...
#   define V(i,x,y,z) arrowVtx [i].push_back(vgm::Vec3(x, y, z))
#   define N(i,x,y,z) arrowNorm[i].push_back(vgm::Vec3(x, y, z)) 
